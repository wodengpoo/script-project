    ...
    for i in range(len(busnm)):
        ...
        if rescode == 200:
            ...
            for item in elements:
                ...
                if busnm[i] != found.text: continue
        ...
        request = urllib.request.Request(url + serviceKey + '&arsId=' + stid + '&busRouteId=' + busid[i])
        response = urllib.request.urlopen(request)
        rescode = response.getcode()
        ...
        if rescode == 200:
            ...
            for item in elements:
                found = item.find("busRouteNm")
                if busnm[i] == found.text:
                    found = item.find("busRouteType")
                    bstype = found.text
                    break

        businfo.append(tuple((gapbustime, busarrivetime, nextsttn, F_TM, L_TM, bstype)))
    return businfo
...
def KgetbusInfo(stid, busid, busnm):
    ...
    for i in range(len(busnm)):
        ...
        request = urllib.request.Request(url + serviceKey + '&routeId=' + busid[i])
        response = urllib.request.urlopen(request)
        rescode = response.getcode()
        ...
        if rescode == 200:
            ...
            for item in elements:
                ...
                # print(found.text)
                if on == False and stid != found.text:
                    continue
                elif on == False:
                    on = True
                    continue
        ...
        businfo.append(tuple((gapbustime, busarrivetime, nextsttn, F_TM, L_TM, F2_TM, gapLT, offset, bstype)))
    return businfo
